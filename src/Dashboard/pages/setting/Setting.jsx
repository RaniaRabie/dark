...
- Description: Component for editing and updating Admin profile information with validation and API integration
*/
...
// UpdateUser component for editing user profile details
function UpdateUser() {
  // State for user information and form controls
  ...
  // @ts-ignore
  const [showImagePreview, setShowImagePreview] = useState(false); // Control image preview visibility
  ...
  const handleSave = async () => {
    try {
      ...
      if (phoneError || !userInfo.phoneNumber) {
        const errorMessage = phoneError || "Phone number is required.";
      ...
      // Validate date of birth
      const dobValidationError = validateDOB(userInfo.dateOfBirth);
      if (dobValidationError) {
        setDobError(dobValidationError);
        setSnackbarMessage(dobValidationError);
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
        return;
      }
...
// @ts-ignore
                Tooltip
                  ...
                  open={editStates.all && (!userInfo.dateOfBirth || !!dobError)}
                  {...tooltipProps}
                >
